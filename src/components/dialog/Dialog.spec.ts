...
test.describe("Dialog", () => {
  ...
  // inside a CodePreview (data-testid="code-preview"); the source block beside it is
  // highlighted <pre> text, so the trigger is scoped through it.
  ...
  test("header rule resolves to the divider border-width token", async ({
    page,
  }) => {
    await activate(openDialog(page));
...
test.describe("DialogConfirm", () => {
  ...
  // is shared with the Usage trigger, so it is scoped through the example's CodePreview.
  const yesNoSection = (page: Page) =>
    page
      .getByTestId("code-preview")
      .filter({ has: page.locator("#dialog-yes-no") });
  const yesNoTrigger = (page: Page) =>
    yesNoSection(page).getByRole("button", {
  ...
  test("closes via its cancel button and returns focus to the trigger", async ({
    page,
  }) => {
    const trigger = yesNoTrigger(page);
    await activate(trigger);
  ...
  test("title resolves to the component-title font tokens", async ({
    page,
  }) => {
    await activate(yesNoTrigger(page));

    const expectedWeight = await resolvedStyle(
      page,
      "font-weight",
      "var(--font-weight-title)"
    );
    const expectedSize = await resolvedStyle(
      page,
      "font-size",
      "var(--text-title)"
    );
    ...
    const title = page
      .locator("#dialog-yes-no")
      .getByText("Confirm Action", { exact: true });
