...
test.describe('Details', () => {
  ...
  // (with icon). Scoped through the CodePreview <section> wrappers.
  const allDetails = (page: Page) => page.locator('section').locator('details');
  const withIcon = (page: Page) =>
    allDetails(page).filter({ hasText: 'Details Heading with Icon' });

  test('summary toggles its content open and closed from the keyboard', async ({ page }) => {
  ...
  test('renders expanded on load when the open prop is set', async ({ page }) => {
    const opened = page.locator('section').locator('details[open]');

    await expect(opened).toHaveCount(1);
    await expect(opened.getByText('Lorem ipsum', { exact: false })).toBeVisible();
  });
  ...
  test('chevron rotates when open and transitions on the overlay duration token', async ({
    ...
    const closedChevron = allDetails(page).first().locator('summary svg.size-5');
    await expect(closedChevron).toHaveCSS('transition-duration', expectedDuration);
  ...
  test('respects prefers-reduced-motion: transitions off, disclosure still opens', async ({
    ...
    await expect(details).toHaveJSProperty('open', true);
    await expect(details.getByText('Details content', { exact: true })).toBeVisible();
  });
});
